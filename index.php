...
// foreach( $orders as $id => $qty)
 ...
 //echo "err occur".$errors;
        echo '<div class="alert-danger text-light" style="background:#c43323; margin:0px; padding:0px;">'.$errors[0].'</div>';
        echo '<label class="alert-danger"><b>Note :</b> If you foget password please contact administrator !</label>';
...
<?php
echo 'Batch 28<br/>'; 
$sqlq = 'SELECT * FROM SMS_MEMBER';
$stmt = sqlsrv_query($con,$sqlq,NULL);
echo '<u>Group Members </u><div class="text-left" style="margin:0px; padding:0px;"><b>';
while($rec = sqlsrv_fetch_array($stmt)){
echo $rec['NAME'].'('.$rec['TYPE'].')<br/>';
}
//&#09;&#09;&ensp;&emsp;&thinsp;
echo '</b></div>';
?>
